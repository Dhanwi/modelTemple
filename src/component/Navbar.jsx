...
const Navbar = () => {
  ...
  return (
    <div className='w-full py-5 sm:px-10 px-5 flex justify-between items-center text-center fixed'>
      <div
        ref={shivRef}
        className='object-cover flex items-center h-10 w-10 sm:w-12 sm:h-12 md:h-20 md:w-20 lg:h-20 lg:w-20 shadow-md shadow-yellow-500 rounded-full hover:shadow-6xl hover:shadow-white  cursor-pointer focus:ring focus:ring-gray-50 mr-8'
      >
      ...
      <div className='flex flex-grow justify-center max-sm:hidden'>
        <div className='flex flex-row items-center '>
          {navLists.map(nav => (
            <div key={nav} className=' ml-2 lg:ml-5 text-xs lg:text-sm cursor-pointer transition-all font-normal lg:font-bold md:font-semibold text-white'>
              {nav}
            </div>
          ))}
        </div>
